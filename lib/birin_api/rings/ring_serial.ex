defmodule BirinApi.Rings.RingSerial do
  ...
  def create_ring_numbers_from_series(%{length: series_length, serial_start: serial_start, serial_end: serial_end} = ring_series, _) when is_map(ring_series) do

    {prefix, start_num} = serial_parts(serial_start, series_length)
    {^prefix, end_num} = serial_parts(serial_end, series_length)

    serial_number_stream(prefix, start_num, end_num)
  end
